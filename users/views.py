...
def password_reset_request(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            messages.success(
                request,
                "Если email зарегистрирован в системе, инструкции по восстановлению "
                "пароля будут отправлены на указанный адрес.",
                extra_tags="password_reset success",
            )
...
def password_reset_confirm(request, token):
    ...
    if request.method == "POST":
        ...
        if new_password and confirm_password:
            if new_password == confirm_password:
                ...
                messages.error(
                    request, "Пароли не совпадают.", extra_tags="password_reset error"
                )
        else:
            messages.error(
                request,
    ...
    return render(
        request, "users/password_reset_confirm.html", {"token": token, "user": user}
    )
...
def add_user_view(request):
    ...
    if request.method == "POST":
        try:
            ...
            messages.error(
                request, f"Произошла ошибка при добавлении пользователя: {str(e)}"
            )
    ...
    if request.method == "POST":
        try:
            ...
            if user.is_superuser:
                messages.error(
                    request, "Нельзя редактировать администратора через интерфейс."
                )
                return redirect("users:users_management")
            ...
            messages.error(
                request, f"Произошла ошибка при обновлении пользователя: {str(e)}"
            )
...
def delete_user_view(request):
    ...
    if request.method == "POST":
        try:
            ...
            messages.error(
                request, f"Произошла ошибка при удалении пользователя: {str(e)}"
            )
