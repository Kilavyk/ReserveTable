...
def _get_booked_slots(selected_date):
    ...
    bookings_on_date = Booking.objects.filter(
        booking_date=selected_date, status__in=["confirmed", "pending"]
...
def _check_booking_availability(table, selected_date, selected_time_slot, booked_slots):
    ...
    is_available = (
        table.id not in booked_slots or selected_time_slot not in booked_slots[table.id]
    )
...
def _validate_booking_data(booking_date, time_slot, table, guests_count):
    ...
    if guests_count > table.max_guests:
        return (
            False,
            f"Количество гостей ({guests_count}) превышает вместимость столика ({table.max_guests}).",
        )

    # Проверка доступности временного слота
    existing_booking = (
        Booking.objects.filter(
            table=table, booking_date=booking_date, time_slot=time_slot
        )
...
def _create_booking_success_message(booking, booking_user, request_user):
    ...
    if booking_user == request_user:
        return (
            f"Бронь столика №{booking.table.number} подтверждена: "
            f"{formatted_date}, {time_display}, {booking.guests_count} гостя."
        )
    else:
        return (
            f"Бронь столика №{booking.table.number} для {booking_user.email} "
            f"подтверждена: {formatted_date}, {time_display}, {booking.guests_count} гостя."
        )
...
def booking_view(request):
    ...
    selected_time_slot, selected_time_slot_display = _parse_time_slot(
        request.GET.get("time_slot")
    )

    # Если время не выбрано, устанавливаем ближайшее доступное
    if not selected_time_slot:
        selected_time_slot, selected_time_slot_display = _get_default_time_slot(
            selected_date
        )

    # Получаем все активные столики, подходящие по вместимости
    suitable_tables = Table.objects.filter(
        is_active=True, max_guests__gte=guests_count
    ...
    if selected_time_slot is not None:
        for table in suitable_tables:
            is_available = _check_booking_availability(
                table, selected_date, selected_time_slot, booked_slots
            )
    ...
    if request.user.is_authenticated and (
        request.user.groups.exists() or request.user.is_staff
    ):
        all_users = CustomUser.objects.filter(is_active=True).order_by(
...
def create_booking_view(request):
    ...
    if request.method == "POST":
        try:
            ...
            # Валидация данных
            is_valid, error_message = _validate_booking_data(
                booking_date, time_slot, table, guests_count
            )
            if not is_valid:
            ...
            success_message = _create_booking_success_message(
            ...
            if email_results["user"]:
                messages.info(
                    request, "Письмо с деталями бронирования отправлено на почту."
                )
            else:
                messages.warning(
                    request,
                    "Бронирование создано, но не удалось отправить письмо с подтверждением.",
                )
            ...
            messages.error(
                request, f"Произошла ошибка при создании бронирования: {str(e)}"
            )
            return redirect("bookings:booking_view")
...
def send_booking_confirmation_email(booking):
    ...
    user_html_message = render_to_string(
        "bookings/booking_confirmation_email.html", user_context
    )
    ...
    admin_context = {
        ...
        "table_description": (
            ...
            if hasattr(booking.table, "description")
            else "не указано"
        ),
        "formatted_date": formatted_date,
        "time_display": time_display,
    ...
    admin_html_message = render_to_string(
        "bookings/booking_notification_admin.html", admin_context
    )
    admin_plain_message = strip_tags(admin_html_message)

    admin_subject = (
        f"Новое бронирование столика №{booking.table.number} - Gourmet Haven"
    )

    email_results = {"user": False, "admin": False}
...
def admin_panel_view(request):
    ...
    for table in tables:
        ...
        tables_data.append({"table": table, "slots": table_slots})
...
def edit_booking_view(request, booking_id):
    ...
    if booking.status == "confirmed":
        messages.error(
            request,
            "Нельзя изменить подтвержденное бронирование.",
    ...
    if request.method == "POST":
        try:
            ...
            if not is_valid:
                ...
                messages.error(
                    request, error_message, extra_tags=f"booking_edit_{booking_id}"
                )
                return redirect("users:profile")
            ...
            messages.success(
                request, f"Бронирование столика №{table.number} успешно изменено!"
            )
            return redirect("users:profile")
            ...
            messages.error(
                request,
                f"Произошла ошибка при изменении бронирования: {str(e)}",
                extra_tags=f"booking_edit_{booking_id}",
            )
            return redirect("users:profile")
