...
class TableAdmin(admin.ModelAdmin):
    ...
    list_filter = ["is_active", "max_guests"]
    ...
    fieldsets = [
        ("Основная информация", {"fields": ["number", "max_guests", "is_active"]}),
        (
    ...
    def get_current_bookings_count(self, obj):
        ...
        return Booking.objects.filter(
            table=obj, booking_date=today, status__in=["confirmed", "pending"]
        ).count()
    ...
    def get_future_bookings_count(self, obj):
        ...
        today = timezone.now().date()
        return Booking.objects.filter(
            table=obj, booking_date__gte=today, status__in=["confirmed", "pending"]
        ).count()
